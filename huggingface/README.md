...
这套文件用于把 Hermes Agent 部署到 **Hugging Face Docker Space**。其中 `/root/.hermes` 是 Hermes 在容器内的实际运行目录，挂载的 `/data` 仅作为 HF Storage bucket 的持久化入口。

## 目录说明

- `Dockerfile`：基于官方 `nousresearch/hermes-agent` 镜像的 HF Space 定制镜像
- `entrypoint.sh`：首启引导脚本，自动初始化 `/root/.hermes` 与 `/data`，并启动 `hermes gateway run`
- `config.space.yaml`：首次启动复制到 `/data/config.yaml`，再恢复到 `/root/.hermes/config.yaml` 的默认配置
- `.env.space.example`：首次启动复制到 `/data/.env`，再恢复到 `/root/.hermes/.env` 的说明模板

## 设计目标

- 无需交互式 `hermes setup`
- Space 重启后自动从 `/data` 恢复状态到 `/root/.hermes`
...
容器启动后，Hermes 实际运行在 `/root/.hermes`；挂载的 `/data` 只负责持久化同步。容器重启时会从 `/data` 恢复到 `/root/.hermes`。

### 当前完整同步清单

| 批次类型 | 路径 | 说明 |
| --- | --- | --- |
| 配置批 | `.env` | API key、渠道凭证、环境变量持久化 |
| 配置批 | `config.yaml` | Hermes 主配置 |
| 配置批 | `SOUL.md` | 角色/人格设定 |
| 配置批 | `state.db` | 主状态数据库 |
| 配置批 | `weixin/accounts/` | 微信账号文件、`*.context-tokens.json`、`*.sync.json` |
| 配置批 | `channel_directory.json` | 聊天通道目录索引 |
| 配置批 | `auth.json` | 认证状态文件 |
| 配置批 | `processes.json` | 后台进程登记状态 |
| 配置批 | `gateway_voice_mode.json` | 网关语音模式状态 |
| 单目录批次 | `sessions/` | 会话数据 |
| 单目录批次 | `memories/` | 长期记忆 |
| 单目录批次 | `plans/` | 计划产物 |
| 单目录批次 | `cron/` | 定时任务状态 |
| 单目录批次 | `workspace/` | 工作区产物 |
| 单目录批次 | `home/` | 子进程 HOME 状态 |
| 单目录批次 | `hooks/` | Hook 脚本/状态 |
| 单目录批次 | `skills/` | 已安装/成长中的技能 |
| 单目录批次 | `skins/` | CLI 主题/皮肤 |
| 单目录批次 | `optional-skills/` | 可选技能目录 |
| 单目录批次 | `plugins/` | 插件目录 |
| 单目录批次 | `scripts/` | 脚本目录 |

### 当前同步策略

- 启动时：从 `/data` 恢复到 `/root/.hermes`
- 运行时：不扫描 `/data`，不做哈希比较，只从 `/root/.hermes` 单向覆盖写回 `/data`
- 周期同步：默认每 `300` 秒同步 1 个批次（可用 `HERMES_PERSIST_SYNC_SECONDS` 调整）
- 退出时：顺序执行全部批次的最终同步
...
因此只要 HF Space 的 Storage 挂载到 `/data`，这些关键数据就会在 `/root/.hermes` 与 `/data` 之间按当前同步清单保持同步，重启后也会尽可能保留。
...
说明：这里的 `/data` 是 **当前 Space 内的 bucket 挂载路径**，不是 Hermes 默认运行目录。Hermes 运行目录仍然是 `/root/.hermes`。
...
HERMES_HOME=/root/.hermes                                          # Hermes 在 HF Space 内的实际运行目录
HERMES_PERSIST_HOME=/data                                          # HF Storage bucket 挂载路径，建议固定为 /data
```
...
1. 设置 `HERMES_HOME=/root/.hermes`
2. 设置 `HERMES_PERSIST_HOME=/data`
3. 创建 `/root/.hermes` 运行目录和 `/data` 持久化目录结构
4. 首次启动时复制模板文件到 `/data`
5. 从 `/data` 恢复当前同步清单到 `/root/.hermes`
6. 同步内置 skills
7. 设置：
...
- 扫码成功后，凭据会先写入 `/root/.hermes/.env`，并按同步策略持久化到 `/data/.env`
...
4. 用户扫码确认后，容器会把返回的 `WEIXIN_ACCOUNT_ID`、`WEIXIN_TOKEN`、`WEIXIN_BASE_URL` 写入 `/root/.hermes/.env`，并同步持久化到 `/data/.env`
5. 之后再启动网关时，就会直接使用已持久化并恢复到 `/root/.hermes` 的微信凭据
