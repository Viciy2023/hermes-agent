...
容器启动后会把 Hermes 的运行态数据保存在 `/data`，并在重启时从 `/data` 恢复到 runtime 目录。
...
- 启动时：从 `/data` 恢复到 runtime 目录
- 运行时：不扫描 `/data`，不做哈希比较，只从 runtime 单向覆盖写回 `/data`
...
因此只要 HF Space 的 Storage 挂载到 `/data`，重启后这些关键数据都会尽可能保留。
...
2. 创建持久化目录与 runtime 目录结构
3. 首次启动时复制模板文件到 `/data`
4. 从 `/data` 恢复当前同步清单到 runtime 目录
5. 同步内置 skills
6. 设置：
